"""N3-PR2 — G5 e G6 dos estados de S1 em quatro colunas (T3-R5): Tab × AVD, paisagem × retrato.

  python3 g5g6-s1.py <dir-dos-dumps>

Os dumps se acham pelo nome: `*-S1-<estado>-<tab|avd>-<pai|ret>.xml`.
...
falhou = False
estados = {}
for f in sorted(glob.glob(os.path.join(d, "*-S1-*.xml"))):
    m = re.match(r".*?-S1-(.+)-(tab|avd)-(pai|ret)\.xml$", os.path.basename(f))
    if m:
...
def alvos(arq):
    ...
    for m in re.finditer(r"<node ([^>]*?)/?>", open(arq).read()):
        ...
        if a.get("package") != "rocks.octavia.app" or a.get("clickable") != "true":
            continue
        b = [int(v) / 2.25 for v in re.findall(r"-?\d+", a["bounds"])]
        out.append((a.get("resource-id", "").split("/")[-1], b[2] - b[0], b[3] - b[1], a.get("enabled")))
    return out


print("G5/G6 — S1, quatro colunas: " + " · ".join(f"{a} {o}" for a, o in COLS))
for est, por in estados.items():
    ...
    for rid, cel in sorted(ids.items()):
        print(f"  | `{rid}` | " + " | ".join(" · ".join(f"{w:.1f} × {h:.1f}" for w, h in cel[c]) if c in cel else "—" for c in COLS) + " |")
