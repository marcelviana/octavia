"""N3-PR5 — G5 e G6 do palco e das provas de "passa" (cópia do g5g6-r-f.py da N3-PR4) em quatro colunas (T3-R5): Tab × AVD, paisagem × retrato.
...
for d in sys.argv[1:]:
    for f in sorted(glob.glob(os.path.join(d, "*.xml"))):
        m = re.match(r"^[A-Z0-9]+-((?:S3|S5|S4|S0|picker|S2-sem-edicao)-.+)-(tab|avd)-(pai|ret)\.xml$", os.path.basename(f))
        if m:
...
print("G5/G6 — palco, S5, S4, S0, picker e S2p, quatro colunas: " + " · ".join(f"{a} {o}" for a, o in COLS))
