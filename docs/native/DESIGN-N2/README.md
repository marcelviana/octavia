...
| div. | o que | o que foi feito |
| --- | --- | --- |
| **319** | **Origem P**: o roteiro pede a N2-E8 como `×` de 48 dp **dentro** do campo, com `testID` `form-limpar-data`. Ela está na `main` desde a N2-PR4: `form-data-limpar`, `Limpar` **ao lado** do campo. Renomear o `testID` faria o G2 reprovar (SUMIU). | Decisão do Marcel: fica como está; o CN (c) usa `form-data-limpar`; fecho na N2-E8 acima, com a remedição nos dois aparelhos. |
| **320** | **Origem P**: a N2-E20 também já estava na `main` (div. 311, N2-PR6): os outros `Adicionar` ficam `enabled=false` durante o request e voltam no `relendo…`. O CN (b) pedido passa contra a `main`. | CN existente + a asserção "sem frase"; o controle ad hoc (tirar o `ocupado`; pôr uma frase no alvo) reprova — "CN que não reprova é instrumento quebrado" vale pelo controle. |
