...
| **333** | **Achado, NÃO consertado — decisão do Marcel.** `IndexScreen.tsx`, `DialogoDeApagar` → `aoSalvoNaoRelido`: apagar com 200 e a releitura falhando sai para S1 com `aoSairParaS1(null)`, e o comentário diz *"Quem mostra isso é S1"* — mas S1 não recebe nada: nenhuma linha de aviso, e a lista (o cache não mudou, N2-D22) continua mostrando a setlist apagada. É "salvo" limpo, que a N2-D22 proíbe. **Lido no código; não alcançado no aparelho nesta PR.** | Registrado. O remédio óbvio é o mesmo desenho da N2-E21 (um terceiro `AvisoDeSaida`), mas pede frase — e frase é do Marcel. |

**Próxima divergência livre: 334.**
