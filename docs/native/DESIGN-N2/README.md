...
> **Origem** — coluna acrescentada no W4-b1 (#321, div. 341), lida do texto de cada divergência: **P** premissa do prompt · **D** doc anterior · **A** ambiente, dado real ou defeito do produto · **T** toolchain/aparato · **X** terceiros (`N1-ENCERRAMENTO.md` §7).
...
|---|---|---|---|
| **234** | P | **Origem P** (o prompt presumiu o que não existia): o prompt manda "seletor de calendário do sistema" como se o módulo já estivesse no projeto. `[medido]` `ls apps/native/node_modules \| grep -i "datetime\|picker\|calendar"` → vazio. É **módulo nativo**. | Decisão do Marcel: entra por `npx expo install @react-native-community/datetimepicker` (**9.1.0**, a do SDK 57, não a 9.2.1 do npm), com o config plugin no `app.json`. **Exige dev client novo nos dois aparelhos antes do §4.** A linha do `android-debug-apk` desta PR ganha a nota "primeiro build com módulo nativo novo desde o N1". |
| **235** | P | O prompt cita `App.tsx:163/177` como os pontos onde o prefetch é chamado. | São **165** e **178** na `main` de hoje (`fedfd24`). Sem consequência: o gancho não entrou em nenhuma das duas, entrou num efeito próprio. |
| **236** | P | O prompt diz que a poda deixa "os outros **quatro**" pendentes. | São **cinco**: `alca`, `renomear`, `apagar-setlist`, `adicionar`, `remover` — seis nomes para cinco registros do anexo D, porque `adicionar / remover` é **um** registro e **duas** entradas no mapa (a própria div. 226 já dizia). O §3 do prompt pedia "0 acusações + 4 avisos"; o medido é **0 acusações + 5 avisos**. |
| **237** | D | O `pode-ter-gravado` da N2-PR2 não serve à folha. | **N2-E1** acima. |
| **238** | T | O `gate:a20` estendido ao `frases.ts` **lia o arquivo e examinava zero literais** (`[medido]`: 24 arquivos, **72** literais — o mesmo número de antes de o arquivo entrar). As posições da varredura são as do JSX e o `FRASES` é chaveado por `rede`, `auth`, `criando`… | Entrou a posição `EXTRAS: valor de chave`, válida só nos arquivos da lista. `[medido]` depois: **100** literais, 0 acusações. Controle negativo ad hoc: três frases trocadas por inglês → duas acusadas, a terceira isenta por anglicismo do produto (`offline`), inclusive em valor multilinha. **Gate que lê e não examina é instrumento quebrado.** |
| **239** | T | `api.ts` lia a base da URL no **carregamento do módulo** (`const BASE_URL = …` no topo). Qualquer teste que importasse `src/` estaticamente congelava a base em `''` antes de o `beforeAll` definir a porta do mock, e toda request falhava por rede com `status=net` e nenhuma pista. `[medido]` no CN do gancho do prefetch. | Passou a ser lida **na chamada** (`baseUrl()`). No aparelho é indiferente: o Metro substitui `process.env.EXPO_PUBLIC_*` por literal no build. Exceção declarada no `g1.sh`. |
| **240** | T | O duplo de `react-native` mapeava `Pressable` para `<button>`, e o app tem um `Pressable` dentro de outro (o `Baixar esta setlist` dentro do cartão de S1) — HTML inválido, aviso do React a cada render. | `<div role="button">`. O que os CNs leem é `data-testid` e `data-disabled`, que não dependem da tag. |
| **241** | T | Uma releitura em voo **sobrevivia ao `desmontar()`** e a linha `resync` dela caía no log do teste seguinte: o CN (f) passava sozinho e reprovava na suíte. | O `afterEach` deixa as pendentes assentarem antes de limpar. **Poluição, não defeito** — e vale registrar porque todo CN de tela das PRs 4–7 tem a mesma forma. |
| **242** | D | O `PRD-TELA-2.md` T2-R1 diz "com o 201 … abre a setlist nova em **S2**"; a legenda de `N2-F-salvando` diz *"a folha fecha e **S1 relê** a lista"*. | **O congelado vence.** Não há navegação para S2 na folha. Os "≤ 3 taps" do J3 continuam: `Nova setlist` · digitar · `Criar`. Registrado no T2-R1. |
| **243** | D | "Criar começa ativo" × `N2-F-validacao`. | **N2-E2**. |
| **244** | D | `form-erro-data` é **inalcançável pelo seletor do sistema**: um calendário nativo não produz `31/02`, que é o que a moldura `N2-F-validacao` desenha. | O guarda do `dataExiste` fica como defesa e o CN o exercita com um `Date` inválido. A moldura continua desenhando um estado que o teclado produziria e o calendário não — **não é erro do desenho**, é consequência da N2-D16/§2 ter escolhido o seletor. |
| **245** | D | `form-falha`, o 26º `testID`. | **N2-E4**. |
| **246** | D | S1f: duas linhas no congelado, uma na implementação. | **N2-E5**. |
| **247** | D | A variante da regra 4 dentro da folha diria "foi criada" sobre o que não foi criado. | **N2-E3**. |
| **248** | D | A releitura da lista **atrás da folha** (regra 3) não tem razão própria no conjunto fechado do T2-R16 (`write\|404\|order\|reopen`). | Usa **`reason=reopen`**, que é a mesma leitura sem `op` que a N2-D22 já nomeia. Alargar o conjunto fechado por causa de uma ocasião nova seria fazê-lo crescer em silêncio, que é o que ele existe para impedir. Fica declarado: **duas ocasiões, uma razão**, e o que as separa no logcat é a linha `write op=` imediatamente antes. |
| **249** | T | Não havia **nenhuma** infraestrutura de teste de tela no repositório: os projetos do Vitest coletam só `.ts` em ambiente `node`, e é por isso que o `PRD-TELA-2.md` repete "a parte da TELA é das PRs 3–7" requisito a requisito. | Entrou o quarto projeto, `native-tela` (`.tsx` em `jsdom`), com o `react-dom` 19.2.3 que **já era** devDependency de `apps/native` — **nenhuma dependência nova** — e duplos para `react-native`, `react-native-svg` e o `datetimepicker`. **Não mede geometria**: os 190 × 57,8 dp e os 48 dp são do dump do aparelho, como em toda esta série. As PRs 4–7 herdam o aparato. |
...
| div. | origem | o que | o que foi feito |
|---|---|---|---|
| **250** | P | O roteiro do §4.1 sobe o mock na **8081**, que é a porta do **Metro**; e o gate `which adb emulator` reprova nesta máquina por `PATH` de shell não-login, com as duas ferramentas instaladas. | Mock na **8788** (a do docstring do `aceite.py` e de todo aceite anterior). O gate virou `ls` no SDK. Erros do roteiro, não do aparato. |
| **251** | T | `expo run:android --device` recusa o serial do `adb` (`RX2N8000F3D`) e o `ro.product.model` (`SM-T865`). | O nome é o do `getDevicesAsync` do Expo: **`SM_T865`**. Fica para as PRs 4–7. |
| **252** | D | `criar-setlist` mede **152,0 × 57,8 dp**, não 190 × 57,8; a caixa do título fica em **534,2 dp**, não 495,8. | **N2-E6** acima. |
| **253** | T | O APK **local** desta série tem **uma** abi (`arm64-v8a`, 82.030.412 B) contra as quatro da V1-PR3 (231.377.892 B). E no CI o `android-debug-apk` deu **13m11s** e **8m9s** em duas pushes cuja árvore difere **só num parágrafo de anexo**. | O tamanho: é o `expo run:android` mirando a abi do aparelho ligado, não o módulo novo — **não se compara com a faixa do CI**. O tempo: a diferença de 5m2s é **cache do runner**, e a leitura de que os 13m11s vinham do módulo novo (div. 234) **fica registrada como errada** — duas medições da mesma árvore não dizem o custo do módulo. A faixa de n=12 da V1-PR6 segue sendo a referência. `aparato.md` §2. |
| **254** | A | No ramo `salvando` o botão `Criar` é renderizado **sem `testID`** (`FolhaDeCriar.tsx:207`); o ramo `editando` passa `form-salvar`. | O **G6 fica sem `resource-id` próprio** para `N2-F-salvando`. O estado é alcançável e o `enabled=false` confere. Correção de outra rodada. **Consertada na N2-PR4** (`a4e7df6`): o ramo `salvando` passa `testID="form-salvar"` em `FolhaDeCriar.tsx:392` naquele commit (`:397` na `main` `bc55419`), com a razão num comentário ao lado. *(Anotação do encerramento do N2, div. 340.)* |
| **255** | X | O seletor de data tem **33 alvos abaixo de 48 dp** — 30 células de dia a 46,2 × 32,0. | São do `DatePickerDialog` do Android, não do app: **custo declarado da N2-D16/§2**, a mesma escolha da div. 244. O G5 dos alvos do app passa com **zero** falhas. |
| **256** | A | **Defeito.** `autoFocus` dentro do `Modal` dá foco e **não sobe o teclado**; o congelado manda subir (e o código cita a frase). Custa um toque a mais, contra os 3 taps do J3. | Registrado, não corrigido. `aparato.md` §5.1. |
| **257** | T | **Defeito de aparato.** O modo `escrita-corta` **não produz falha no Android**: um 201 de corpo truncado passa por sucesso (`await response.text()` não estoura como no Node). | O estado `N2-F-falhou` foi alcançado com `escrita-500`. A espécie `rede` da N2-D18 e a frase `pode-ter-gravado-folha` da N2-E1 **seguem sem prova de aparelho**. `aparato.md` §5.2. |
| **258** | P | O §4 do prompt pede uma **N2-E6** para o `form-falha` fora das 25 — que **já é a N2-E4** (div. 245), palavra por palavra. | **Não foi aberta uma segunda errata do mesmo fato.** A N2-E4 ganhou a prova de aparelho que lhe faltava (dump `07`, 654,2 × 87,6 dp). O rótulo **N2-E6** ficou com a medida do botão. |
| **259** | T | `grep -rn eyJ` sobre os anexos acha **o enunciado da própria regra 4**, no `README.md:121`. | Registrado. Nenhum arquivo novo do §4 contém o literal. |
...
| div. | origem | o que | o que foi feito |
|---|---|---|---|
| **273** | D | O T2-R16 declarou `resync … reason=order` desde a N2-PR2, e **nada o emitia**. | É a releitura da regra 3 depois de um reorder que FALHOU (`relerPelaOrdem`, `escrita.ts`): `resync kind=setlists reason=order op=reorder …`, medida no Tab S6. A remoção da N2-PR4 usa `reason=reopen` no mesmo papel — as duas formas convivem; unificar é decisão do Marcel. O G3 não muda (a linha já existia). |
| **274** | P | **Extra, declarado antes do commit 1**: um módulo novo no core, `ordem.ts` (`mover`, `mesmaOrdem`, `TETO_DO_REORDENAR`, `alvoDoArrasto`), com 16 testes. | O cabeçalho do `escrita.ts` manda regra para o core; o teto de 100 é do contrato e o "nada mudou" é a N2-D36. Duas exceções a mais no G1 (`ordem.ts`, `index.ts`). |
| **275** | D | **Extra**: o voltar do sistema dentro do modo. O congelado não diz. | Vale `Cancelar` / `Sair sem salvar` (sem request); durante o salvamento é engolido — o `Cancelar` some de propósito (regra 1) e sair da tela não cancelaria a escrita. CN próprio. |
| **276** | D | **Contestação da N2-D36**, pedida pelo roteiro. A legenda de `N2-S2e-ordem-falhou` diz que *"'nada mudou → não envia' vale aqui como vale no formulário"* — e no formulário o botão fica **inativo com o motivo** `nada mudou desde que você abriu`, que já está no conjunto fechado. Essa leitura também não pede frase nova, e cumpre a N2-D23. | Implementada a N2-D36 como veio (fecha o modo, sem request, `reason=nada-mudou`). A alternativa é uma troca pequena no `ModoDeReordenar.tsx`; **decisão do Marcel**. |
| **277** | D | Um `testID` serve dois rótulos, duas vezes: `reordenar-sair` é `Cancelar` e `Sair sem salvar`; `reordenar-salvar` é `Salvar a ordem`, `Tentar de novo` e `Tentar recarregar`. | É o `form-salvar` da folha ("o mesmo alvo nos dois modos"): o alvo é o mesmo, o G6 lê o rótulo. |
| **278** | D | `reordenar-motivo` — o 30º `testID` além da tabela do §7. | O motivo do `Tentar de novo` inativo enquanto relê (`relendo…`); pela mesma razão do `form-salvar-motivo`. |
| **279** | D | O toque no `Reordenar` inativo **por teto** loga (`write blocked op=reorder reason=ceiling`, A-N2-9); por **sem rede** ou **escrita em voo**, não. | A faixa da N2-PR4 não loga toque em inativo, e o T2-R16 diz que `write blocked` é "toque num controle de escrita inativo". Só o teto tem aceite que pede a linha; os outros ficam como a PR-4 deixou, declarado. |
| **280** | D | Alças do salvamento inteiras × anexo D. | **N2-E12**. |
| **281** | D | `Reordenar` à esquerda × legenda de agrupamento. | **N2-E11**. |
| **282** | T | **O `gate:icones` era cego a entrada escrita em várias linhas.** O coletor lê cada estado de UMA linha; a primeira forma da `alca` (várias linhas) passou com 0 acusações e "4/6 cobrados", e **o `apagar-setlist` da N2-PR4 estava assim desde que entrou** — só o `inerte` dele era comparado. | Regra nova `[legível]`: entrada sem `normal` numa linha é ACUSADA. O `apagar-setlist` foi posto numa linha e **passa** (o desenho estava certo — quem dizia isso era a sorte, não o gate). CN ad hoc: 2 acusações, exit 1; `IconesFalso` segue em 22. Extra declarado antes do commit 2. |
| **283** | D | A linha `N2-X-100` desenha o ícone `n.º de músicas` em `lineInfo` e o texto em `text`. | A `LinhaDeAviso` pinta ícone e texto com uma cor só, e `lineInfo` nunca é texto (§3.3): vai `muted`, como o "salvo, não relido". |
| **284** | D | Opacidade 0,5 do salvamento × E3. | **N2-E13**. |
| **285** | D | R1·7b põe o alvo de 48 × 72 *"encostado na borda esquerda"*; a moldura desenha o ícone a 24–48 dp dentro da linha (o recuo de 24 da linha). | Segue a R1·7b: o alvo começa na borda da linha e o ícone fica centrado nele, a 12–36 dp. O número cai no mesmo x da moldura (64 dp), porque o vão de 16 começa no fim do alvo. |
| **286** | A | **Defeito, achado no §4.** A linha erguida era translúcida (o acento a 12% no lugar do fundo opaco) e o buraco tracejado aparecia DENTRO dela. | **Consertado**: cópia opaca desenhada como o último filho da lista, acento como camada; a linha que segura o toque fica montada e invisível. CN endurecido, reprova contra o commit 2. `N2-PR5-anexos/aparato.md` §3. |
| **287** | D | A releitura da regra 3 pode falhar DEPOIS de um reorder que falhou; o congelado do modo não tem moldura para isso. | O botão principal vira `Tentar recarregar` (relê; não reescreve), como a N2-D32 prescreve e a N2-E3 fez na folha. |
| **288** | D | Sem rede **dentro** do modo: o congelado não desenha. | A linha `sem-rede-s2` aparece no modo e o botão principal inativa; o arrasto continua possível (nada é escrito durante o gesto). |
| **289** | D | **Tensão, não resolvida aqui.** Um 400 no reorder (a setlist mudou atrás do modo: outra aba adicionou ou removeu) lê a frase do T2-R15 *"a setlist mudou — a ordem foi recarregada"* — mas pela R2·1 a tela **mantém o arrasto**, e o arrasto já não é permutação: `Tentar de novo` recebe 400 de novo, e a saída é `Sair sem salvar`. | A frase promete uma recarga que a R2·1 proíbe de mostrar. Opções para o Marcel: (a) no 400 o modo descarta o arrasto e volta à ordem relida (a frase fica verdadeira); (b) outra frase. Nada foi mudado. |
| **290** | P | O §4.1 pôs o **Tab S6 em modo avião** para o estado sem rede. A regra herdada da V1-PR3 e o T2-R12 dizem que, no Tab S6, "sem rede" é só o override da API. A N2-PR4 fez o mesmo (§7.4 do aparato dela). | Restaurado, `ping` de volta em 16,5 ms. A regra e a prática das duas PRs divergem; se a regra vale, o estado 11 se refaz com o override. |
| **291** | T | As alças não entram na população do G5 (o `View` com `panHandlers` não é `clickable` para o `uiautomator`), e o `remover-7` aparece com 48,0 × 8,9 nos dumps `08` e `11`. | As alças se medem à parte: **48,0 × 72,0**. O `remover-7` é recorte de rolagem (a linha de aviso empurra a grade), não alvo pequeno. |
| **292** | A | O AVD `octavia_tab32` estava em modo avião (e sem wi-fi nem dados). | Lido antes; avião desligado e `svc data enable` para o §4.2(b); restaurado (`airplane=1 data=0`, `ping` inalcançável). |
| **293** | D | O rótulo `movida de <n>` só aparece depois de uma FALHA (a moldura); depois de soltar e antes de salvar, não. | A moldura só o desenha em `N2-S2e-ordem-falhou`. Se o Marcel o quiser antes, é uma condição a menos. |
| **294** | T | **Aparato**: o Metro com `CI=1` não vigia arquivo, e duas tentativas de conserto da div. 286 rodaram o bundle velho — mediram nada. | Achado por `curl` do bundle; Metro sem `CI=1`; todo reteste começa pelo `curl`. |
...
| div. | origem | o que | o que foi feito |
|---|---|---|---|
| **295** | P | O roteiro pede o `code` do contrato para a permutação inválida. **Não existe um próprio**: o `OB601` é SQLSTATE interno da RPC e não sai do servidor; na rede o erro é `400 VALIDATION_ERROR` com `details[].field = "order"` — o mesmo `code` do filtro de nome e dos erros de schema. | A chave é `VALIDATION_ERROR` no `op=reorder`, que o core já chamava de `ordem-mudou` (`packages/core/src/escrita.ts:275`): nada novo é lido da resposta. No `reorder` os outros caminhos até esse `code` (uuid malformado, mais de 100 itens, duplicata, corpo > 1 MB com `field:""`) são inalcançáveis pelo app, que só envia permutação da própria leitura, ≤ 100, e ~4 KB. |
| **296** | D | Com o motivo `nada mudou desde que você abriu` na barra, o título do modo encolhe e trunca (`REORDENAR · ENSAIO DE R…`, PNG `12`). A moldura não previa motivo na barra. | Aceito como está: o título é o nome da setlist, que a tela anterior mostra inteiro, e o motivo é o que a N2-D23 exige. Se o Marcel preferir o motivo em outro lugar (segunda linha da barra), é troca de estilo. |
| **297** | P | O CN pedido afirma `aviso-acao` ausente depois do 400 — mas no modo o `Tentar de novo` nunca morou na linha de aviso: ele é o `reordenar-salvar` da barra. A asserção pedida passaria também contra o código de ANTES. | O CN afirma as duas coisas: `aviso-acao` ausente **e** `reordenar-salvar` diferente de `Tentar de novo` — a segunda é a que reprovava. |
| **298** | P | "Sem `Tentar de novo`" deixa aberto o que o botão principal vira depois do descarte. | `Salvar a ordem`, cheio, pela N2-D36 — inativo com o motivo até o próximo movimento, e então salva o arrasto novo, feito sobre a ordem relida. |
| **299** | P | O roteiro manda a regra do avião para "a seção do aparato do `CLAUDE.md`, se ela existir". | `[medido]` `grep -n -i aparato CLAUDE.md` → nada. Foi para o `PRD-TELA-2.md` §8, como o roteiro prevê, com uma nota no T2-R12. |
| **300** | P | "Anexos em pt-BR, traduza o que estiver em inglês": a varredura dos anexos commitados acha inglês **só em saída literal de ferramenta** (`Test Files … passed`, `AssertionError: expected …`, `Cannot find module`, `Network is unreachable`). | Nenhuma medição foi reescrita — a regra `[medido]` exige a saída literal. O `N2-PR5-anexos/README.md` ganhou um glossário pt-BR dessas linhas. Todo texto que não é saída de ferramenta já estava em pt-BR. |

### Erratas da N2-PR6 (o picker)

**N2-E15 — a reticência da R1·5 é do TEXTO, e conta pontos de código.** A R1·5 manda o placeholder `Adicionar a <setlist>` *"truncado por ellipsis acima de 34 caracteres de nome"*. O placeholder de um campo não tem `numberOfLines`: a reticência entra na string, montada no core (`placeholderDoPicker`, `TETO_DO_NOME_NO_PICKER = 34`) — 34 cabem inteiros, 35 viram os 34 primeiros e `…`. A conta é por **ponto de código** (`[...nome]`), não por unidade UTF-16: um emoji na fronteira não é partido ao meio (teste de unidade). `[medido]` CN (b): 34 → `Adicionar a Ensaio geral da turnê de primavera`; 35 → o mesmo com `…`. No Tab S6, dump `02`: `Adicionar a Ensaio do picker`.

**N2-E16 — duas chaves novas no conjunto fechado e sete construtores, zero redação nova.** `falhou-adicionar` (*"não entrou na setlist"*, o título da linha que falhou em `N2-P-resultados`) e `adicionada` (o estado da linha depois do 201). As redações do §5 que carregam DADO — o placeholder, o vazio (*"Digite para achar na biblioteca e adicionar a **<nome>**."*, em três pedaços porque o nome vai em destaque), *"<n> músicas disponíveis."*, as duas réguas do S4, a marca *"já na setlist · n×"*, o total e o *"nesta visita"* — são função no `frases.ts`, pela razão do `perguntaDeApagar` (div. 227), e o `gate:a20` as lê (`EXTRAS: literal de template`; 142 → 163 literais examinados, 0 acusações). `adicionada`, `adicionando` e `relendo` ficam em minúscula no conjunto; a inicial maiúscula da moldura é estilo (div. 310).

**N2-E17 — o campo mede 900 × 48 com texto e 960 × 48 no vazio.** O §5 diz *"mesmo campo de 900 × 48"*. `[medido]` Tab S6: **900,0 × 48,0** com texto (dump `03`, idêntico ao C5 do `MEDIDAS.md`) e **960,0 × 48,0** no vazio (dump `02`). O campo é o do S4, e no S4 o `apagar` mora DENTRO da caixa e só existe quando há texto: sem ele, o campo cresce os 48 do alvo e os 12 do vão. A moldura desenha o `apagar` fora da caixa, e por isso a caixa dela não muda. **Nada a corrigir**: o picker usa o campo do S4, que é o que o §5 pede, e o S4 tem o mesmo comportamento desde a V1-PR6 — pelo CÓDIGO (`SearchScreen.tsx`, o `apagar` condicionado ao texto); o S4 no vazio não foi medido nesta PR. `[div. 315]`

**N2-E18 — o rodapé com o aviso mede 112,9 dp, não 112.** A legenda de `N2-P-relendo` faz *"48 + 64 = 112 dp de rodapé"*. `[medido]` Tab S6, dump `08`: **48,9 + 64,0 = 112,9**. A barra de 64 conta o fio por dentro (div. 313, consertado); a linha de aviso é o componente de 48 dp do §3.3 e **não se deforma** para caber o fio que a separa da lista — os 0,9 dp (2 px no Tab S6) são esse fio, por cima dela. A moldura, em `border-box`, desenha o fio dentro dos 48. Deformar o componente das duas telas para fechar uma soma seria o erro de ordem inversa da N2-E9. `[div. 316]`

### Divergências abertas na N2-PR6, **301 a 318**

| div. | origem | o que | o que foi feito |
|---|---|---|---|
| **301** | P | **Origem P**: o roteiro cita um `adicionarMusica` do core (PR-2). Ele não existe. `[medido]` `grep -rn adicionarMusica apps/native packages/core/src` → vazio. A adição é `pedidoAdicionar(setlistId, contentId)` (`packages/core/src/escrita.ts:147`, só `{content_id}`) + `escrever(pedido, estado, opcoes)` (`apps/native/src/escrita.ts`). Espécie por caso: 201 → releitura → `ok` ou `ok-nao-relido`; 404 → releitura → `sumiu`; 400 → `servidor` (`dados-recusados`); 500 → `servidor`; 401 → `auth`; 429 → `limite`; rede → `rede` com `podeTerGravado`. Duas chamadas sobrepostas: a segunda é **aceita** com a releitura da primeira em voo, e volta **`busy`** com o REQUEST da primeira em voo (os três CNs da trava, `escrita.test.ts:455-520`). | Usado como está. O "as outras linhas seguem ativas" do congelado é exatamente o escopo da trava da div. 232. |
| **302** | D | **extra-1**: `aoResponder` no `escrever()`. O congelado manda a linha virar *adicionada* **com o 201** e o `k` contar **201 confirmados**; o `escrever()` só devolve depois da releitura. | Retorno opcional, chamado com a classificação preliminar entre o request e a releitura; não decide nada. Declarado antes do commit 1. |
| **303** | D | *"Relendo… é estado próprio, entre adicionada e a marca nova."* A releitura começa no mesmo tick do 201, e o *adicionada* de antes dela não chega a ser desenhado. | O que se vê é `adicionando… → relendo… → adicionada` (com a marca nova), e o `k` sobe no `relendo…` — medido no CN (d) e no Tab S6 (dump `08a`: `Relendo…` com `k` = 2 e total 9). `linha-do-tempo.txt`. |
| **304** | P | O roteiro diz *"`Concluir` e `fechar` → zero requests, S2 relê"*; a legenda de `N2-P-vazio` diz que a volta cai em S2 *"que relê a lista"*; a de `N2-P-relendo`, que *"sair com o aviso aberto é permitido: S2 relê na volta"*. | S2 já tem o conjunto da ÚLTIMA releitura (cada adição o entrega pelo `aoReler`, o caminho da div. 270) — a volta normal não pede nada (Tab S6: zero linha `OCTAVIA:`). Saindo com o aviso aberto, S2 mostra o *"salvo, não relido"* e relê (`reason=reopen`), como a legenda. |
| **305** | D | **extra-3**: a linha de aviso do rodapé também para **sem rede** (`sem-rede-s2`) e para o **teto de 100** (`teto-100`). O congelado só desenha a da releitura falha. | T2-R12 (motivo escrito para o inativo) e T2-R6 (*"o picker avisa que, acima de 100, a setlist não pode ser reordenada"*). Um aviso por vez: sem rede > releitura falha > teto. Tab S6, dump `11`. |
| **306** | D | **extra-4**: `picker-vazio`, `picker-rodape`, `picker-total` além da tabela do §7. | A razão do `form-falha` (N2-E4): sem id o G6 não diz que o vazio e o rodapé foram alcançados, e o CN não lê a tinta do total. |
| **307** | A | Um 404 numa adição com a setlist **ainda** no conjunto relido: quem sumiu foi a música da biblioteca (`404 Content not found`, outro aparelho a apagou). O `classificar` dá `sumiu-setlist` para `add`, e a frase mentiria. | A linha fica em *falhou* com a genérica (`não foi possível salvar`) e **sem** `Tentar de novo` — não há o que repetir. Nenhuma frase nova. Sem CN próprio (o do 404 com a setlist sumida é o (g)). |
| **308** | D | **Tensão, para o Marcel.** Na falha de REDE de um `add` a escrita pode ter passado (N2-D18); o título da moldura, *"não entrou na setlist"*, afirmaria o contrário. | O título vira o `pode-ter-gravado` (*"pode já ter sido gravada — confira antes de repetir"*), e a frase do servidor segue embaixo. **Mas a frase da espécie `rede` do T2-R15 é *"sem conexão — nada foi salvo"***, e as duas lado a lado se contradizem (Tab S6, dump `07`). A frase do T2-R15 é anterior a esta PR e vale para as seis operações; para `create` e `add` ela promete o que a N2-D18 não garante. Não mexida aqui. |
| **309** | D | **extra-2**: `export` do `Regua` no `SearchScreen.tsx`. | *"As mesmas réguas em mono"*: a régua do S4, não uma cópia. |
| **310** | D | A moldura escreve `Adicionando…`, `Relendo…`, `Adicionada` com inicial maiúscula; o conjunto tem `adicionando…` e `relendo…` desde a N2-PR2. | `textTransform: 'capitalize'` na linha do picker; o conjunto fica com uma frase cada (N2-E16). Tab S6: `Adicionando…`, `Relendo…`, `Adicionada` no dump. |
| **311** | D | **Tensão.** A moldura `N2-P-resultados` desenha o `Adicionar` (com `2×`) **ativo** na mesma tela em que outra linha está em `Adicionando…`. Com o REQUEST de outra em voo, o toque voltaria `busy` (div. 232), e a frase da espécie seria *"sem conexão — nada foi salvo"*, mentira com a rede de pé. | Durante o REQUEST os outros alvos ficam **ocupados** (`enabled=false`) — o T2-R11 e o precedente da S2 (N2-PR4) —, e também `Concluir`/`fechar` (sair esconderia uma falha que ainda não aconteceu). Durante o `relendo…`, **ativos**, como o congelado manda. Tab S6, dump `06` × dump `08`. Se o Marcel preferir o desenho, é trocar a trava por uma fila, que a N2-D2 recusou. |
| **312** | D | 429 numa adição. O congelado não desenha. | A linha fica em *falhou* com a frase do prazo e **sem** botão nem releitura — o que falta é tempo (a mesma decisão da linha de aviso de S2, N2-PR4). O gate da família já fecha as outras escritas. |
| **313** | A | **Defeito, achado no §4**: o rodapé media **64,9** — o fio num invólucro fora dos 64. | **Consertado** no commit do aceite; remedido 64,0. CN endurecido, reprova contra o commit 2 (`CN-controles.txt`, CN-4). `N2-PR6-anexos/aparato.md` §3. |
| **314** | T | **Instrumento.** Dois CNs do commit 1 não podiam passar em implementação nenhuma: o número da linha lido no começo do `textContent` (o DOM junta os nós sem espaço: `"1Manhã…"`) e o `Adicionar` do 100 → 101 procurado na linha já *adicionada*. E o `gates.test.ts` afirmava o pendente que o commit 1 podava. | Os dois CNs corrigidos no commit 2, com a razão no próprio `it`; o `gates.test.ts` (22 → 23 acusações; o `it` do "único pendente" dá lugar ao da lista vazia) foi para o commit 1, onde a poda está. Os três controles negativos ad hoc do `CN-controles.txt` provam que os CNs de ordem, de "seguem ativas" e da porta do palco pegam o que guardam. |
| **315** | D | Campo 960 × 48 no vazio. | **N2-E17**. |
| **316** | D | Rodapé de 112,9 com o aviso. | **N2-E18**. |
| **317** | T | **Aparato**: no dev client, o `apagar` do campo fica sob o botão flutuante (engrenagem); o toque abriu o menu de desenvolvimento e o `input text` seguinte foi para a raiz. Nada se perdeu. | O campo passou a ser limpo por `KEYCODE_MOVE_END` + `DEL`. Só existe no dev client. |
| **318** | P | O aceite do T2-R6 (A-N2-6) pede **10** adições na conta de audit; o roteiro desta PR pediu **2** em prod, na principal. | Feitas as 2. As dez seguidas estão no CN (sobrepostas, (d)) e no mock (§4.1), não em prod — o A-N2-6 fica **parcial**, declarado no T2-R6. |

### Erratas da N2-PR7 (estados transversais, aceite completo)

**N2-E19 — a espécie `rede` se parte em duas: quem não enviou e quem não ouviu resposta.** Decisão do Marcel sobre a div. 308. A frase do T2-R15 para rede, *"sem conexão — nada foi salvo"*, era verdade para a escrita que **nem saiu** (`write blocked reason=offline`) e afirmação sem base para a que **saiu e não voltou** (o prazo da N2-D35, o socket cortado) — que é justamente o caso da N2-D18, em que o `POST` pode ter passado. Ao lado do *"pode já ter sido gravada"*, as duas frases se contradiziam na mesma linha (dump `07` da N2-PR6). **Uma chave nova, `sem-resposta`** = *"sem resposta do servidor"*, com a razão escrita no `frases.ts`; `rede` fica com a frase de sempre e passa a valer **só** para o barrado offline. A segunda linha *"pode já ter sido gravada — confira antes de repetir"* acompanha só a `sem-resposta`, e só em criar e adicionar. O barrado passou a ser classificado no core (`classificarBarrado`): offline **nunca** liga o `podeTerGravado`; `busy` diz a genérica (extra-1, div. 323). No picker, com rede, a primeira linha é a frase e a segunda a dúvida (extra-3, div. 325). `[medido]` Tab S6 e AVD, dump `18`: `sem resposta do servidor | pode já ter sido gravada — confira antes de repetir | Tentar de novo`, com `write op=add … status=net code=net ms=20015` e `ms=20029`. É a primeira mudança de asserção no core desde o G1b — daí o G1b com pares (div. 321).

**N2-E20 — com um `POST` em voo, os outros `Adicionar` do picker ficam inertes SEM frase.** Decisão do Marcel sobre a div. 311. **Exceção declarada à regra 11 da folha** (*"inativo é `lineInfo` com o desenho amputado e o motivo escrito ao lado, legível sem toque"*): o motivo é a linha `Adicionando…` ao lado, e escrevê-lo em cada alvo seria o ruído que o congelado recusa em `N2-X-sem-rede`. Durante o `relendo…` eles voltam a ativos, como o congelado manda. Já era o comportamento da `main` desde a N2-PR6 (div. 320); o CN ganhou a asserção "sem frase" e o controle ad hoc prova que ela pega (`N2-PR7-anexos/CN-controles.txt`, (b′)).

**N2-E8, fechada.** O controle de limpar a data entrou na N2-PR4 como está descrito acima — `form-data-limpar`, ícone `fechar` + rótulo `Limpar`, **ao lado** do campo, só quando há data — e o roteiro da N2-PR7 o presumiu aberto (div. 319). Decisão do Marcel: fica como está. Medida da N2-PR4 (**109,3 × 48,0 dp**) **remedida nesta PR nos dois aparelhos**, idêntica: Tab S6 e AVD, dump `03`; o toque esvazia o campo (`dd / mm / aaaa`), o alvo some no mesmo toque, o `Salvar` ativa, o `PUT` leva `performance_date: null` e a tela relê (dump `04`, `write op=update … 200` + `resync … 200`); sem data, o alvo não existe (dump `17`).

**N2-E21 — o 404 com a releitura falha: sai para S1, sem dizer que a lista é a do servidor.** O congelado não previa o caso. O 404 é conhecimento (o servidor disse que a setlist não existe) e o T2-R10 manda sair; mas a frase do 404, `sumiu-declarado`, afirma *"a lista abaixo é a que o servidor tem agora"* — e não houve lista. Decisão do Marcel: **uma chave nova, `sumiu-nao-relido`** = *"Essa setlist não existe mais. Não foi possível recarregar a lista."* — as duas PRIMEIRAS orações de `sumiu-declarado` e de `salvo-nao-relido-s1`, cortadas na fronteira da oração, zero palavra nova (teste de prefixo literal no core). A segunda oração de `salvo-nao-relido-s1` afirmaria o contrário do caso (*"ela pode não aparecer abaixo ainda"* é de uma setlist CRIADA; a apagada pode CONTINUAR aparecendo até o recarregar). S1 mostra a frase com `Tentar recarregar`; relida, a frase inteira do 404 volta a ser verdade e o botão some. Nos cinco caminhos de 404 de S2 (extra-5, div. 328). `[medido]` estados `13` e `14`, nos dois aparelhos. `[div. 327]`

**N2-E22 — a barra do reordenar com o motivo: o título trunca, o motivo não.** O roteiro chamou isto de "N2-E15", rótulo que já é da reticência do picker (div. 322). Fecha a div. 296 como **regra**: com o motivo `nada mudou desde que você abriu` na barra do modo, o título `Reordenar · <nome>` trunca por reticência (`numberOfLines={1}`) e o `reordenar-motivo` **nunca** trunca — o nome da setlist está inteiro na tela anterior, e o motivo é o que a N2-D23 exige legível. Nenhuma linha de código muda: é o que o `ModoDeReordenar.tsx` já faz desde a N2-PR5 (PNG `12` daquela PR).

### Divergências abertas na N2-PR7, **319 a 333**

O aceite do §3 está em [`N2-PR7-anexos/aparato.md`](../N2-PR7-anexos/aparato.md).

| div. | origem | o que | o que foi feito |
|---|---|---|---|
| **319** | P | **Origem P**: o roteiro pede a N2-E8 como `×` de 48 dp **dentro** do campo, com `testID` `form-limpar-data`. Ela está na `main` desde a N2-PR4: `form-data-limpar`, `Limpar` **ao lado** do campo. Renomear o `testID` faria o G2 reprovar (SUMIU). | Decisão do Marcel: fica como está; o CN (c) usa `form-data-limpar`; fecho na N2-E8 acima, com a remedição nos dois aparelhos. |
| **320** | P | **Origem P**: a N2-E20 também já estava na `main` (div. 311, N2-PR6): os outros `Adicionar` ficam `enabled=false` durante o request e voltam no `relendo…`. O CN (b) pedido passa contra a `main`. | CN existente + a asserção "sem frase"; o controle ad hoc (tirar o `ocupado`; pôr uma frase no alvo) reprova — "CN que não reprova é instrumento quebrado" vale pelo controle. |
| **321** | A | **Origem A**: o G1b ("só adição nos testes do core") não tinha saída para uma decisão do core que muda de propósito; a N2-E19 é a primeira. | Decisão do Marcel: **pares declarados** no `g1.sh` — tripla `velha → nova · razão`, a nova entrando IGUAL, a razão no script, o aviso de par não usado —, o mesmo mecanismo do G3 (W4-a). Commit próprio antes do commit 1, CN ad hoc em `g1b-cn.txt` (sem par reprova antes e depois; com par passa; nova diferente, velha apagada, par sem razão e lista fora de triplas reprovam). Linha no `LOGS-OCTAVIA.md`. |
| **322** | P | O roteiro chama de **"N2-E15"** o fecho da div. 296; o rótulo N2-E15 é da reticência do picker (N2-PR6). | O fecho vai como **N2-E22**. |
| **323** | P | **extra-1**, declarado antes do commit 1: com a frase `rede` restrita ao offline, o barrado por `busy` precisava de outra. | A genérica (*"não foi possível salvar"*), sem a dúvida — nada foi enviado. Nenhuma tela chega a esse caso: todas ocupam os controles durante a escrita. |
| **324** | A | **extra-2**: o barrado era classificado no app fingindo uma falha de transporte (`networkError: 'barrado: …'`). | `classificarBarrado(op, motivo)` no core, com o tipo `MotivoBarrado`; o `resultadoBarrado` do app só o chama. O cabeçalho do `escrita.ts` manda regra para o core. |
| **325** | A | **extra-3**: no picker, com rede, o título era o `pode-ter-gravado` e o motivo a frase da espécie. | Inverte: a frase da espécie em cima, a dúvida embaixo ("a frase + a segunda linha", como a N2-E19 manda). |
| **326** | T | O CN (d) do A-N2-15 por linha exata **passa** contra a `main` — o comportamento estava certo, o instrumento é que era frouxo (prefixo e subcadeia). | O controle positivo (leitura: exatamente `OCTAVIA: auth-failure`) e os ad hoc mostram as duas coisas: uma linha `auth-failure` na escrita reprova os quatro CNs, e uma `write auth-failure-guard` passa no exato e **reprova** a forma de subcadeia da `main` — um logout que não houve. O comentário do `api.ts` ("o corpo é vazio de propósito… conteria a subcadeia") segue verdadeiro, com a razão mais fraca; não mexido. |
| **327** | A | **Defeito da N2-PR4, achado no §3.1 no AVD**: com a escrita e a releitura da regra 3 falhando juntas, S2 dizia *"a lista abaixo é a que o servidor acabou de devolver"* e oferecia `Tentar de novo` (contra a N2-D32); e um 404 com a releitura falha saía para S1 com a frase de lista relida. | **Consertado**, decisão do Marcel, commits `9268c66` (5 CNs reprovando) e `fb9acd5`: sem a oração e com `Tentar recarregar`; o 404 sai com a **N2-E21**. Remedido nos dois aparelhos (estados `12`–`14`). |
| **328** | A | **extra-5**, declarado antes do commit do conserto: o buraco do 404 sem releitura não era só do `remover` — os outros quatro caminhos de saída de S2 (editar, apagar, reordenar, picker com a setlist sumida) chamavam `aoSairParaS1('sumiu')` com `setlists === null`. | `AvisoDeSaida` com `'sumiu-nao-relido'` nos cinco; CN do editar. O picker com a MÚSICA sumida não muda (div. 307). |
| **329** | T | **Instrumento**: nenhum modo do mock alcançava "a escrita falha E a releitura falha". E o próprio CN do conserto, na primeira forma, lia o `GET` do mock no modo que o faz falhar. | Sufixo `-releitura-500`, que compõe com qualquer modo de escrita; o CN parte do `setlistComBis()`. Os 5 CNs reprovam de novo com os quatro arquivos de `ba3836c` no lugar. |
| **330** | T | **Arnês**: no AVD o teclado encaixado cobre a metade de baixo — o `form-cancelar` e os `Adicionar` de baixo; o toque cai numa tecla (`E`, `p`). E `input text` fora do campo recarrega o dev client (`r`). | `BACK` com o teclado de pé, conferido pelo `mInputShown`, antes de procurar alvo. **Em prod, o `p` custou zero escrita** (o toque não achou `Adicionar`); a retomada conferiu campo e total antes de continuar. |
| **331** | P | **Origem P**: o §3.3 pede *"10 adições numa visita (3 + 9 toques, uma delas bis)"*. Numa setlist criada vazia, isso é impossível: dentro de uma visita a linha adicionada fica `adicionada`, sem botão, e não há bis de música que ainda não estava lá. | 9 distintas numa visita (3 + 8 toques, uma rolagem) e o bis numa **segunda** visita (3 toques): as mesmas 10 adições, as mesmas **13 escritas**, e o A-N2-6 inteiro ("≤ 3 taps cada, sem sair de S2; bis gera duas posições"). |
| **332** | P | O J3 do roteiro conta *"1 toque nas seguintes"* e *"reordenar (1 arrasto + 1 toque)"*. | Medido no Tab S6: 1 toque da 2ª à 4ª; **a 5ª pede uma rolagem** (as adicionadas sobem para "Nesta setlist" e o teclado flutuante da Samsung fica no meio da tela); reordenar = **3 gestos** (entrar, arrastar, salvar); abrir a setlist nova é 1 toque de navegação, fora da conta. O tempo de parede (73,2 s) é quase todo do arnês (~67 s de dumps e pausas); o lado do app somou 1,29 s. |
| **333** | A | **Achado lido no código na N2-PR7, CONSERTADO na mesma PR** (decisão do Marcel). `IndexScreen.tsx`, `DialogoDeApagar` → `aoSalvoNaoRelido`: apagar com 200 e a releitura falhando saía para S1 com `aoSairParaS1(null)` — nenhuma linha de aviso, e a lista (o cache não muda sem releitura, N2-D22) continuava mostrando a setlist apagada: o "salvo" limpo que a N2-D22 proíbe. | **N2-E23** abaixo. CN antes (`da69bb2`, 2 reprovando: a saída vinha `[null]` e S1 mostrava a setlist sem `aviso-motivo`), conserto e aparelho no commit seguinte. |
| **334** | T | **Método do CN da div. 333**: a `Navigation` real (`@react-navigation`) não roda no duplo `jsdom`. | O CN mede as duas metades, como o CN (j) do 404: a saída de S2 (`{ apagadaNaoRelida: 'Show' }`) e o S1 montado com o aviso. O mapeamento entre elas (`navigation.tsx`) está provado **só no aparelho** (dumps `19`, nos dois). O modo `delete-releitura-500` não pediu código no mock: é o sufixo `-releitura-500` da N2-PR7 sobre o comportamento normal. |
