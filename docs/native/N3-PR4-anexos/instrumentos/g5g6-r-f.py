"""N3-PR4 — G5 e G6 do reordenar, da folha e do diálogo (cópia do g5g6-s2.py da N3-PR3) em quatro colunas (T3-R5): Tab × AVD, paisagem × retrato.

  python3 g5g6-r-f.py <dir-dos-dumps> [<dir> ...]

Os dumps se acham pelo nome: `*-<reordenar|folha|dialogo>-<estado>-<tab|avd>-<pai|ret>.xml`.
...
for d in sys.argv[1:]:
    for f in sorted(glob.glob(os.path.join(d, "*.xml"))):
        m = re.match(r"^[A-Z0-9]+-((?:reordenar|folha|dialogo)-.+)-(tab|avd)-(pai|ret)\.xml$", os.path.basename(f))
        if m:
...
print("G5/G6 — reordenar, folha e diálogo, quatro colunas: " + " · ".join(f"{a} {o}" for a, o in COLS))
