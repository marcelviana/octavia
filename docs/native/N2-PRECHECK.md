...
**Posse.** Com `getSupabaseServiceClient()` (`lib/supabase-service.ts:7`, chave
`SUPABASE_SERVICE_ROLE_KEY`) e a policy `"Service role access to setlist songs"`
(`schema.dump.sql:485`; a de `setlists` é `:489`), **a RLS não protege nenhuma
destas rotas: a posse é garantida apenas pelo handler.** As oito usam esse
cliente (`route.ts:22,96`; `[id]/route.ts:36,169,337`; `songs/route.ts:21`;
`order/route.ts:23`; `[songId]/route.ts:38`). Nas três rotas de RPC, a
checagem de dono acontece **na rota, antes e fora da transação**, e as funções
não recebem uid. `[análise]` Isso é estável porque nenhum writer muda o
`setlist_id` de uma música (só o INSERT o define: `grep 'set setlist_id'` sem
ocorrência, `C-posse.txt`). **Só a W3 fica sem linha de posse (div. 150).**
`[medido: C-posse.txt]`

| # | Método e rota | Handler | Cadeia | Bearer sem cookie (linha do extrator) | Exige email verificado | Schema Zod | Rate limit | `updated_at` | Lock | Respostas | Posse (filtro por usuário) |
|---|---|---|---|---|---|---|---|---|---|---|---|
| W1 | `POST /api/setlists` | `route.ts:91-216`, export `:224` | **B** | sim — `secure-auth-utils.ts:267-270` (regex `/^Bearer\s+(.+)$/i`; o cookie só é lido no `else`, `:277`) | **sim** (`:307-310`) | `setlistSchemas.create` (`lib/api-schemas.ts:339-350`), `.strict()`, `songs[]` ≤100 sem `content_id` repetido | `setlist-mutate` `MUTATE` 120/15 min (`route.ts:92`); conta **antes** da validação do corpo (`middleware:114-125` < `:145-164`) | o handler preenche `created_at` e `updated_at` com `new Date()` do Node no INSERT (`route.ts:133-134`) | nenhum (setlist nova); com `songs[]`: INSERT multi-row (`:168-171`) e, se falhar, um DELETE que desfaz a criação (`:174-178`) | 201 = setlist + `setlist_songs` gravadas; 400 `VALIDATION_ERROR` (middleware `:163`; posse de `songs` `:117-119`); 401/429 (middleware); 500 (`:182`, `:213`) | **sim**: posse dos `songs[]` em `route.ts:104-108` (`.in('id', ids).eq('user_id', user.uid)`) + contagem `:114`; a linha nova nasce com `user_id: user.uid` (`:132`); o DELETE que desfaz filtra `:178` |
| W2 | `PUT /api/setlists/[id]` | `[id]/route.ts:147-294`, wrapper `:297-310` | **B** | sim — idem | **sim** | `setlistSchemas.update` (`api-schemas.ts:354-357`), `.strict()`, todos os campos opcionais; `{}` passa e só atualiza o `updated_at` | `setlist-mutate` (`:148`) | **atualizado no handler**, no mesmo UPDATE (`:174-176`, `new Date()` do Node) | **nenhum explícito**: é um UPDATE simples (`:195-201`), sem pré-condição e sem `If-Match` — quem grava por último vence | 200 = setlist + songs com content embutido; 400 id (`:163-166`); 404 (`PGRST116`, `:207-209`); 500 (`:291`) | **sim**: o próprio UPDATE filtra `.eq("user_id", user.uid)` (`[id]/route.ts:199`); o content embutido na resposta, `:249` |
| W3 | `DELETE /api/setlists/[id]` | `[id]/route.ts:315-367`, wrapper `:370-381` | **A** (`:320`) | sim — `firebase-server-utils.ts:153-155` (`startsWith('Bearer ')`, diferencia maiúsculas) | **não** | **nenhum** de corpo; o id passa por `commonSchemas.objectId` (`:332-335`) | `setlist-mutate` via `enforceUserLimit` (`:325`) | — (a linha é apagada) | **nenhum**. Apaga `setlist_songs` **sem filtro de dono** (`:340-343`) e depois `setlists` com `user_id` (`:351-355`); FK `ON DELETE CASCADE` (`schema.dump.sql:468`) → **div. 150** | **200 `{success:true}` mesmo quando nada foi apagado** (**div. 151**); 400 id; 401; 429; 500 (`:365`) | **NÃO no DELETE de `setlist_songs`** (`[id]/route.ts:340-343`, sem linha) → **div. 150**; só o DELETE de `setlists` filtra (`:355`) |
| W4 | `POST /api/setlists/[id]/songs` | `songs/route.ts:13-86`, wrapper `:89-101` | **B** | sim | **sim** | `setlistSchemas.addSong` (`api-schemas.ts:371-380`); `position` é aceita e ignorada (`route.ts:65`) | `setlist-mutate` (`:14`) | atualizado **dentro da RPC**, na mesma transação (`schema.dump.sql:67`) | `FOR UPDATE` na linha-pai (`dump:49`) | 201 = a linha inserida (6 colunas); 404 `Setlist not found` (`:35-37`) / `Content not found` (`:50-54`); OB602 → 404 (`lib/rpc-errors.ts:40-42`); 500. **O id do path não passa por validação de uuid** → **div. 153** | **sim, no gate da rota**: setlist `songs/route.ts:28`, content `:47`; a RPC `add_setlist_song` **não recebe uid** (`dump:42`) |
| W5 | `PUT /api/setlists/[id]/songs/order` | `order/route.ts:17-62`, wrapper `:65-75` | **B** | sim | **sim** | `setlistSchemas.reorder` (`api-schemas.ts:363-369`): 1..**100** uuids, sem repetição | `setlist-mutate` (`:18`) | na RPC (`dump:337`) | `FOR UPDATE` (`dump:287`) | 200 `{songs:[{id,position}]}`; 400 `field:"order"` (OB601, `rpc-errors.ts:24-31`); 404; 500. Id do path sem validação → div. 153 | **sim, no gate da rota**: `order/route.ts:31`; a RPC `reorder_setlist_songs` **não recebe uid** (`dump:277`); a permutação só aceita songs da própria setlist (`dump:300`) |
| W6 | `DELETE /api/setlists/songs/[songId]` | `[songId]/route.ts:22-92`, wrapper `:95-106` | **A** (`:27`) | sim — `firebase-server-utils.ts:153-155` | **não** | **nenhum**; `songId` vai cru para `.eq("id", songId)` (`:52`) → div. 153 | `setlist-mutate` (`:32`) | na RPC (`dump:265`) | `FOR UPDATE` (`dump:225`) | 200 `{success:true}`; 404 `Song not found` (`:59-61`, `:70-72`, OB602/OB603); 500 | **sim, no gate da rota**: join `setlists!inner (id, user_id)` (`[songId]/route.ts:47-50`) + comparação `songData.setlists.user_id !== user.uid` → 404 (`:70-72`); a RPC `remove_setlist_song` apaga **só pelo `p_song_id`** (`dump:208`, `:232`) |
| R1 | `GET /api/setlists` (leitura) | `route.ts:12-86` | A | sim | não | — | `setlist-read` (`:19`) | — | — | 200 array; 401; 429; 500 | **sim**: `.eq('user_id', user.uid)` (`route.ts:37`); o content embutido segue a FK **sem** filtro de dono (delta declarado, `:28-32`). Mesmo cliente de service role (`:22`) |
| R2 | `GET /api/setlists/[id]` (leitura) | `[id]/route.ts:13-128` | A | sim | não | id por `objectId` (`:31`) | `setlist-read` (`:23`) | — | — | 200; 400; 404; 500 | **sim**: setlist `[id]/route.ts:43`; songs lidas por `setlist_id` só depois desse gate (`:63-67`); content `:86`. Mesmo cliente (`:36`) |
...
1. **O corpo da B-c não foi commitado** (div. 162). O `B-c-1.txt` traz os **headers
...
| **159** | **P** | **Origem: o prompt da Fase B**, que disse "o mesmo mecanismo das Fases B do C e do N1", e os dois mecanismos não são o mesmo. **"O mesmo mecanismo das Fases B do C e do N1 (`scripts/ux-audit/`)" são dois mecanismos.** O C usou o `auth.ts` inteiro, que depois do `signInWithPassword` faz `POST /api/auth/session` e recebe cookie (`scripts/ux-audit/auth.ts:109-139`; `C-PRECHECK.md:1163`: "o único POST foi o `/api/auth/session` do `signIn`"). O N1 usou um script próprio **só** com o `signInWithPassword` (`N1-PRECHECK-anexos/A3-probe-n1.mts.txt`). Usei o do N1: o do C somaria uma request a prod fora do orçamento e um cookie que os probes não podem ter | registrada; §11 |
| **160** | **P** | **A B-d do aval pressupõe duas coisas que a rota não tem.** (1) "setlist real da B-c": `DELETE /api/setlists/songs/[songId]` **não recebe id de setlist** (`[songId]/route.ts:95-106`); (2) "se a B-a der 401, a B-d vai devolver 401 também": a removeSong é da **cadeia A** (`:27`, `requireAuthServer`), que **não** exige email verificado; ela daria 500 mesmo para uma conta não verificada. **A posse é conferida em `[songId]/route.ts:70-72`** (join `setlists!inner (id, user_id)` em `:47-50`, antes da RPC de `:79`, que apaga só pelo `p_song_id`, `dump:208,232`) `[medido: C-posse.txt]` | registrada; a B-d rodou com `songId=nao-e-uuid` |
| **161** | — | **Não atribuída.** O complemento pré-merge reservou a 161 para "remover música sem filtro de dono", e a medição mostrou que o filtro existe (div. 160, §1.1 W6). O número fica vago de propósito, para a 162 manter o número que o prompt deu | — |
| **162** | **P** | **O prompt da Fase B pediu o corpo da B-c em anexo, e o precedente do N1 é não commitar `content_data`/`file_url`.** O precedente está escrito em `N1-PRECHECK.md:365` (*"Os corpos JSON de prod (`setlists.json` 49.983 B, `content-100.json` 52.941 B) **não** são anexos (contêm corpo de música)"*) e no cabeçalho do probe do N1 (`N1-PRECHECK-anexos/A3-probe-n1.mts.txt:3`: *"Nunca imprime token/email/corpo de música"*). Registrado em `B-c-1.txt`: headers literais, resumo do corpo, bytes (49.983) e sha256 (`08ebfe43…e8cb01`) | registrada; §11.2 item 1 |
...
| `C-posse.txt` | complemento pré-merge: posse por rota, RPC `remove_setlist_song`, precedente do N1 (divs. 160/161/162) |
