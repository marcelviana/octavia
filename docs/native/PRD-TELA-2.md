...
> **Este PRD prepara, não decide.** O que as decisões vigentes não fecham vai para o §12 como pergunta numerada, com opções e recomendação. Nenhum requisito abaixo depende de uma resposta do §12, a não ser onde está escrito "depende de Q*n*".
> **Divergências**: de **177 a 185** (a #306 usou até 162; a #307, até 176).
...
**T2-R1 — Criar setlist** `[J3 passo 1 e critério "≤ 3 taps"; N2-D1; SETLISTS.md §POST]`. De S1, criar uma setlist pede **nome** (obrigatório) e **data** (opcional) e envia um `POST /api/setlists` com `{name}` ou `{name, performance_date}`, **sem `songs[]`** (as músicas entram pelo picker, T2-R6; ver Q8). Os campos `description`, `venue` e `notes` não são enviados (ausente grava `null`, `route.ts:127-131`). Com o 201, o app ressincroniza (T2-R9) e abre a setlist nova em S2, no estado vazio que orienta o próximo passo (J3, ponto de observação). Da lista S1 até a setlist criada, **≤ 3 taps**, sem contar a digitação do nome.
...
**T2-R3 — Validação no cliente antes de enviar** `[pre-check §7.1 ("`errorFrom` descarta `details`"); div. 154; div. 181; nota N1]`. O cliente valida **antes** do request, porque o 400 chega à tela sem o campo (`errorFrom` descarta `details`) e porque todo 400 consome `setlist-mutate` (§3). Regras, iguais às do servidor: (i) nome com `trim()` e 1..255 caracteres; (ii) nome, sem diferenciar maiúsculas, **não pode conter** `data:`, `javascript:`, `vbscript:` nem `<script` — a tela diz qual trecho impede e por quê, em pt-BR, sem mencionar segurança; (iii) a data sai do seletor, então é sempre um dia que existe (a div. 154 fica inalcançável pelo nativo). O botão de enviar fica inativo, com o motivo visível (critério do `DESIGN-V1` §8), enquanto a entrada não passa.
*Aceite*: teste de unidade do validador com os casos da nota N1 (`Show — data: 12/10` recusado; `Datas de outubro` aceito; `"   "` recusado; 255 aceitos, 256 recusados); no aparelho, digitar `Show — data: 12/10` → nenhum request (`grep 'write op=create'` vazio) e o motivo aparece.

**T2-R4 — Renomear e mudar a data** `[J3; SETLISTS.md §PUT; C4]`. Editar o nome ou a data envia um `PUT /api/setlists/[id]` **só com os campos que mudaram** (ausente = não mexe). Tirar a data envia `performance_date: null`. O nome nunca é enviado como `null`. **O web regrava os cinco campos sempre** (`setlist-service.ts:187-194`, pre-check §4); o nativo não copia, porque isso apagaria em silêncio um `venue`/`notes` que o nativo não exibe. O corpo do 200 **não** é aplicado (C5); vale a ressincronização (T2-R9).
...
**T2-R8 — Reordenar: um request por gesto concluído** `[J3 passo 3 e critério "drag-and-drop … ou controles equivalentes"; C8; div. 155; T1-R28]`. Reordenar em S2 (arrastar, ou controles equivalentes, a decidir no brief) envia **um único** `PUT /api/setlists/[id]/songs/order` **quando o gesto termina**, com a permutação completa dos `setlist_songs.id` — nunca um request por passo do arrasto. Durante o gesto, a ordem mostrada é a do gesto; depois do 200, vale a ordem do servidor. O 200 do reorder **é** leitura da ordem (C8), mas não do `updated_at` (§4.1): a substituição segue a regra do T2-R9. **Teto de 100** (div. 155): numa setlist com mais de 100 músicas, reordenar não é oferecido e a tela diz por quê (Q5). **Caso de medida**: a setlist de 60 da conta de audit (`4340bf95…`, pre-check §11).
*Aceite*: na setlist de 60, mover a 8 para a posição 2 com um gesto → **exatamente uma** linha `api … path=/api/setlists/<id8>/songs/order` e uma `write op=reorder items=60` no log; depois do resync a 8 antiga está em "2 de 60"; em mock com 101 músicas o controle de reordenar está inativo com o motivo visível e nenhum request sai.

**T2-R9 — A verdade é o servidor: ressincronizar depois de todo 2xx** `[§4; T1-R9; T1-R10; N2-D8; H-N2-5; depende de Q1]`. Depois de todo 2xx de escrita, o app faz a leitura escolhida na Q1 (recomendada: `GET /api/setlists`, opção b) e **substitui** o conjunto de setlists pelo 200 dela. Enquanto essa leitura não volta, a tela mostra a operação como **concluída no servidor e ainda não relida** (o texto é do brief). Se a leitura falhar, o cache **não** muda (T1-R9), a tela mostra a falha de sincronização (T1-R18) e a escrita **não** é desfeita nem repetida. Nenhuma escrita altera o cache diretamente.
*Aceite*: teste de unidade: dado um cache com a setlist X de `updated_at` T0 e um 201 de addSong, o cache só muda quando a leitura pós-escrita chega, e fica igual ao 200 dela (`cmp` do `setlists.json` com o corpo sem embutido); com a leitura pós-escrita em 500, o `setlists.json` é byte a byte o anterior e aparece a falha; `grep` no código de escrita por gravação de `setlists.json` fora do caminho do resync → vazio.
...
**T2-R11 — Falha aparece; nada de "salvo" antes do 2xx; sem repetição automática** `[N2-D2; T1-R37; H-N2-4; C10]`. Toda escrita tem três estados visíveis: **salvando** (request em voo), **falhou** (não-2xx ou rede, com a frase do T2-R15) e **concluída** (2xx). "Salvo", ou qualquer sinal equivalente, **só** depois do 2xx. **Nenhuma** escrita é repetida automaticamente (C10: `POST` repetido duplica). Enquanto uma escrita está em voo, os outros controles de escrita da mesma tela ficam **ocupados** (uma escrita por vez; isso não é fila: nada é guardado para depois). "Tentar de novo" depende de Q6.
*Aceite*: mock com atraso de 3 s → "salvando" visível durante o atraso e nenhum "salvo"; mock com 500 → "falhou" visível, `setlists.json` inalterado (`cmp`), **uma** linha `api` para a rota (nenhuma repetição); com uma escrita em voo, tocar em outro controle de escrita não gera request.
...
**T2-R14 — 429 numa escrita** `[T1-R4; §3]`. Um 429 de `setlist-mutate` bloqueia **todas** as escritas (a família é compartilhada) até o `Retry-After`, com a mensagem pt-BR do T1-R4. As leituras não são bloqueadas (outra família).
...
| ressincronização | `resync kind=setlists\|full reason=write\|404\|order op=<op> status=<s\|net> setlists=<n\|-> ms=<ms>` | fim da leitura do T2-R9/R10 (`kind` depende de Q1) |
...
**T2-R17 — Criar ou datar para os próximos 7 dias dispara o prefetch** `[N2-D11; T1-R15; T1-R17; A10; div. 182]`. Quando a leitura do T2-R9 traz uma setlist com `performance_date` entre hoje e hoje+7 (a janela de `selectPrefetch`, `core/offline.ts:90-102`), o prefetch do T1-R15 roda para ela **sem** o usuário abrir a setlist, e o indicador do T1-R17 é recalculado. Com a opção (c) da Q1 isso já acontece (`App.tsx:159`); com a (b), a ressincronização chama o mesmo `prefetchEArrumar`. **O que o aceite mede é o mecanismo** (N2-D11): o A10 com dado real da conta principal continua **não reproduzido** até o Marcel criar uma setlist de show de verdade (H-N2-8; decisão de 2026-09-11).
...
**T2-R19 — Edição e palco não se cruzam** `[H-N2-7; pre-check §8.3 item 2; T1-R24; depende de Q7]`. O palco é endereçado por `position` (`navigation.tsx:31`), e um reorder muda a música que uma posição aponta. Enquanto houver um palco na pilha da mesma setlist, a edição dessa setlist em S2 **não é oferecida** (recomendação da Q7), ou o palco passa a ser endereçado por `setlist_songs.id`.
*Aceite*: palco na música 4, abrir o índice pelo palco → nenhum controle de edição; voltar ao palco → "4 de N" com a mesma música (`stage restore n=4/N`).
...
| A-N2-3 | Nome inválido (nota N1) não gera request e mostra o motivo | teste de unidade; `grep 'write op=create'` vazio | T2-R3 |
| A-N2-4 | `PUT` só com o campo mudado; `venue` do servidor sobrevive ao renomear | corpo no mock; leitura pós-resync | T2-R4 |
| A-N2-5 | Apagar com confirmação; 404 (já apagada) volta a S1 sem `auth-failure`; arquivos intactos | log; `cmp files-index.json` | T2-R5 |
...
| A-N2-9 | Setlist de 101 (mock): reordenar inativo com motivo, zero request | captura; log `write blocked … reason=ceiling` | T2-R8 |
...
| A-N2-21 | Palco aberto: índice sem edição; volta ao palco na mesma música | log `stage restore n=4/N`; captura | T2-R19 |
...
1. **Entrada de "criar" em S1** — onde fica (Q4) e como ela aparece no estado vazio **S1f**, cujo texto hoje diz para criar "na versão web" (`SetlistsScreen.tsx:359-361`) e passa a ser falso com o N2 (pre-check §8.3 item 1).
2. **Formulário de nome e data** — criar e editar; o seletor de calendário; "sem data"; o motivo quando o nome não passa (T2-R3).
3. **Modo de edição em S2** — entrar e sair; reordenar (arrastar ou controles equivalentes, J3); remover por posição; o estado da setlist acima de 100 (T2-R8).
4. **O picker** — busca na biblioteca local, "adicionar" repetível sem fechar, marca de "já está na setlist" (Q3), e o aviso acima de 100 (Q5).
5. **Confirmação de apagar** — o que ela diz e em quantos toques (Q2).
6. **Estados da escrita** — salvando, concluída e ainda não relida, falhou (com as frases do T2-R15), sem rede, limite de taxa, e "tentar de novo" (Q6).
...
| **G2** (`g2g3.sh`) | `testID` novos: entrada de criar, formulário, modo de edição, alça ou controle de reordenar e de remover por posição, picker e seus resultados, confirmação de apagar, os estados da escrita. Nenhum dos 43 atuais some (pre-check §8.2) | PRs da tela |
...
| **G6** | todo estado novo alcançável por `resource-id` e todo alvo tocável com `testID` — inclusive salvando, falhou, sem rede, limite, teto de 100, confirmação | aceite da tela |
| **G7** (`g7.sh`) | os testes de unidade do T2-R2, R3, R9, R15 e o CN do T2-R18 | cada PR |
| **`gate:icones`** | ícones novos (criar, editar, arrastar, remover, apagar, calendário) entram no mapa contra o design congelado do N2 | PRs da tela |
...
| **H-N2-4** (herdada) | nenhuma repetição automática; o risco de setlist ou bis duplicado sob rede instável é aceito | Marcel (Q6) | resposta da Q6 |
| **H-N2-5** (herdada) | a leitura pós-escrita é `GET /api/setlists` (opção b) | Marcel (Q1) | resposta da Q1 |
| **H-N2-6** (herdada) | `performance_date` impossível → 500 | executor | a Fase C ou um teste de rota; o nativo não gera esse valor (T2-R3) |
| **H-N2-7** (herdada) | reordenar com o palco na pilha muda a música de uma posição | executor / Marcel (Q7) | T2-R19 |
...
| **H-N2-15** (nova) | nenhum nome de setlist real do Marcel contém `data:` ou `on<palavra>=` em `notes`/`description` (o web já os recusaria) | Marcel | o T2-R3 impede no nativo; a correção do refine é do Bloco D (div. 181) |
...
| Div. 154 — `performance_date` sem checagem de calendário | pre-check | **Bloco D** (inalcançável pelo nativo, T2-R3) |
| Div. 155 — reorder com teto de 100, addSong sem teto | pre-check | **Bloco D** (o nativo declara o teto, T2-R8, Q5) |
...
| **181** | **A** | **O `refine` de texto recusa português comum.** `/data:/i` (`api-schemas.ts:61`, `:71`, `:79`) recusa um nome `Show — data: 12/10` ou um `venue` `Bar Data: centro`; o `/on\w+\s*=/i` do `safeHtml` (`:79`) recusa `notes: "dois tons = abaixo"`. `[medido: nota N1]` | registrada; o nativo valida antes (T2-R3); corrigir o refine é **Bloco D**; H-N2-15 |
