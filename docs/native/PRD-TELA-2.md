...
| **N2-D36** | nova (N2-PR5) | **`Salvar a ordem` com a ordem inalterada fecha o modo SEM request** — mesmo efeito do `Cancelar` — e loga `write blocked op=reorder reason=nada-mudou`. **Nenhuma frase nova**: a N2-D21 continua com as três validações. Decisão pré-tomada no roteiro da N2-PR5 e implementada como veio; a leitura contrária da legenda de `N2-S2e-ordem-falhou` ("'nada mudou → não envia' vale aqui como vale no formulário", onde o formulário deixa o botão **inativo com o motivo**) está registrada na div. 276 para o Marcel decidir. `[medido: N2-PR5]` CN (d) do `reordenar.test.tsx`; no Tab S6, uma linha `write blocked … nada-mudou` e zero request. | T2-R3, T2-R8 |
