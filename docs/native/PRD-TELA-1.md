...
- **Base de referência**: 3 setlists / 69 songs / 66 content / 5 arquivos da conta de audit `[medido: C-PRECHECK B.2, B.3]`; o repertório real do Marcel é da mesma ordem de grandeza `[hipótese: não medido — a biblioteca da conta principal não foi lida; um `GET /api/content?pageSize=100` com a conta real, ou o `total` do dashboard, fecha]`.
...
Envelope de erro comum a todas as rotas `[contrato: CONTRATO-DE-ERRO.md]`: `{ error, code, details? }`, `code` ∈ {`AUTH_REQUIRED` 401, `VALIDATION_ERROR` 400, `NOT_FOUND` 404, `RATE_LIMITED` 429, `INTERNAL_ERROR` 500}, mapeamento 1:1 com o status; 429 traz `retryAfter` no corpo e `Retry-After` no header `[medido: lib/user-rate-limit.ts:125-137, citado em C-PRECHECK §4.1/B.5-4]`. Não-2xx cujo corpo não parseia como envelope (405 vazio, 404 HTML de rota inexistente, 413 text/plain) = erro genérico, sem retry `[contrato: cláusula não-JSON]`.
...
**T1-R4 — Tratamento de 429** `[contrato: CONTRATO-DE-ERRO.md RATE_LIMITED; medido: lib/user-rate-limit.ts:125-137]`. O cliente lê `Retry-After` (segundos, inteiro; também em `retryAfter` no corpo) e **não** reenvia antes desse prazo; a UI mostra "servidor ocupado, tentando em N s" (regra C3-1: toda falha aparece). Com os orçamentos do §6 o 429 é teoricamente inalcançável na tela 1; o requisito é de robustez.
*Aceite*: resposta 429 simulada (mock) com `Retry-After: 30` → nenhuma request à mesma família nos 30 s seguintes; mensagem visível.

**T1-R5 — Email verificado** `[medido: §1.3]`. Nenhuma rota da tela 1 exige email verificado (todas na cadeia A). Registrado porque as rotas da cadeia B (`/api/profile` GET/PATCH, `/api/storage/*`, todas as mutações via `withBodyValidation`) exigem — a tela 2 herdará isso. A tela 1 **não chama** `/api/profile`; a existência ou não de perfil não a bloqueia.
*Aceite*: conta com email não verificado abre a tela 1 e lê setlists/content (200) — verificável com uma conta de teste nova `[hipótese: não há conta assim hoje; criar uma é escrita fora do escopo deste PRD]`.
...
**T1-R9 — Substituição sem merge** `[C-D4; PLANO SET-14 (fila A #5): "deleções em outro dispositivo não ressuscitam"]`. Uma resposta 200 de `GET /api/setlists` **substitui** o conjunto inteiro de `setlist`+`song` do usuário; uma resposta completa de `GET /api/content` (todas as páginas até `hasMore == false`) substitui o conjunto de `content`. Nada é mesclado; o que não veio, some. Arquivos (`file`) **não** são apagados na substituição (são imutáveis por URL; limpeza é por LRU, T1-R14).
*Aceite*: cache com 4 setlists; servidor devolve 3 → a UI lista 3; a 4ª não existe mais localmente.

**T1-R10 — Versionamento por `updated_at`** `[C-D4]`. Cada gravação compara `updated_at` do item novo com o local: igual → não re-renderiza nem re-baixa; diferente → substitui e invalida derivados (índice de busca, layout renderizado). `updated_at` de `content` é bumpado pelo servidor em todo PUT `[medido: app/api/content/route.ts:255, citado em §2.6]`.
...
3. Depois: prefetch de arquivos (T1-R15) em background, concorrência ≤ 3 (o web usa 3 `[medido: lib/advanced-content-cache.ts:158, §2.5]`).
4. Ao voltar do background com > 30 s ausente ou ao recuperar rede: repetir 2 (o web usa 30 s `[medido: hooks/use-setlist-data.ts:172, §2.5]`).
...
| `content-read` | 300/min | ⌈N/100⌉ = 1 hoje | 0,3% |
| `proxy` | 120/min | **0** (C-D2) | 0 |
| `session` | 120/15min | **0** (C-D1) | 0 |
| bucket direto | sem família | ≤ nº de `file_url` não cacheadas (5 hoje) | — |

Latência de referência `[medido: B.2]`: 150–340 ms por request quente; 1,5–2 s nos dois primeiros hits da campanha (`[hipótese 12: cold start]`). Payload: 49.983 B + 52.941 B ≈ 103 KB por abertura.
*Aceite*: (a) em modo avião, com cache populado, a lista de setlists aparece em < 1 s após o launch e a setlist de 60 músicas (`UX-AUDIT Estresse` `[B-P6]`) abre completa; (b) online, a captura de tráfego de uma abertura mostra exatamente 2 requests a `/api/*` (com ≤100 content) e nenhuma a `/api/proxy`; (c) primeira abertura sem cache mostra "sincronizando…", nunca "sem setlists", até o primeiro 200.
...
**T1-R25 — Renderização por tipo** `[C-D7 (§4); PLANO C4 "renderer cobrindo TODOS os content types desde o design"; CONT-01/02]`. Lyrics, Chords-texto e Tab renderizam como texto monoespaçado/`pre` (Tab com as 6 cordas alinhadas — CONT-02); Chords-arquivo e Sheet renderizam o arquivo. **O TAB desligado do web é defeito do web** (`use-content-loading.ts` só lê `chords`/`sections` `[medido: §2.6]`), não requisito herdado: no nativo, Tab renderiza desde o primeiro build.
...
| H14 | Repertório real do Marcel na mesma ordem de grandeza da conta de audit (§0) | Marcel | ⌈N/100⌉ requests de content por abertura; busca local ainda cabe até ~5 MB de corpos (C-D3) | `total` de `GET /api/content` com a conta principal |
...
| A4 | Abertura online: exatamente 2 requests a `/api/*` (biblioteca ≤ 100) e lista visível do cache em < 1 s | T1-R13 |
