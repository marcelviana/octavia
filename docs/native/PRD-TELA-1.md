...
**T1-R20 — Índice local** `[C-D3; PLANO LIB-04 (B11 sai do caminho do nativo); J5]`. Campos indexados: `title`, `artist`, `album` (os mesmos do ILIKE do servidor `[medido: §2.3]`) **mais** o corpo de texto (`lyrics`/`chords`/`tablature` do contrato §4) — o corpo é barato (19.481 B no total `[medido: B.3]`) e resolve "lembro de um verso". Índice reconstruído por item a cada invalidação (T1-R10). **Errata (W4-a, div. 194):** o app reconstrói o índice **INTEIRO** quando qualquer item invalida — `buildIndex` roda sobre o conjunto, não sobre o item. A N2-PR1 poupou só o caso "nada mudou" (`invalidated=0` reaproveita o `contentById`). A frase "por item" descreve um comportamento que nunca existiu; o custo medido do índice é pequeno (19.481 B de corpo) e mudar isto é tela (S4), não registro.
...
**T1-R26 — PDF e imagem do cache** `[C-D2; PLANO C2 "PDF no palco" (item 4, fechado na web pelo PERF-02); C4 "PDF: caso menor, due diligence na 1ª semana"]`. Arquivo é renderizado do disco (T1-R14); se não estiver no cache e não houver rede → placeholder "arquivo não baixado" com ação "baixar" (J6 ponto "placeholder claro ou quebra silenciosa"). PDF de 12 páginas (`[medido: B.2 P7 — file_url …partitura-12p.pdf; 242.176 B por B5-PRECHECK §2.2, referência]`) rola por página. **Errata N1-PR8 (2026-09-11, `N1-ENCERRAMENTO.md` §6)** — o render usa `fitPolicy={0}` (**fit width**, a partitura ocupando a largura da tela). A N1-PR6 tinha posto `{2}` (página inteira, **um** gesto = **uma** página); o aceite no Tab S6 mediu que assim a A4 fica em ~548 px de 2560, **~244 dp de largura** — ilegível no palco `[medido: N1-PR7 §3.6]`. Decisão do Marcel (2026-09-10): legibilidade ganha de contagem de gestos. **Limitação conhecida, registrada, não defeito**: com fit width o deslize rola *dentro* da página antes de virar, então virar uma página custa vários gestos — o **T1-R27** ("avançar: 1 tap ou 1 gesto") **não se cumpre para a PÁGINA do PDF**. Ele continua valendo para a **MÚSICA**: as bordas de 15% avançam e voltam em 1 tap, em PDF como em texto.
...
> **Errata do T1-R27 / A14 (N3-PR5, 2026-09-25; N3-D13).** Com as faixas do N3, **a rotação do tablet muda a FORMA da barra superior do palco**: em paisagem (faixa C) ela é a de 64, uma linha; em retrato (faixa B, 711 dp) é **88 em duas linhas** — posição + setlist em cima, título · artista · tipo embaixo, com 663 dp. As bordas continuam **entre as barras** (D-1) e com **15 % da largura** do conteúdo (106,7 dp em B), então a altura delas muda com a barra (869,8 dp no AVD em retrato); a barra inferior é a mesma. **A posição continua preservada**: girar na música 3 dá `rotation=portrait n=3/8` e volta com `rotation=landscape n=3/8`, com `3 DE 8` nos dois dumps, no Tab S6 e no AVD (`N3-PR5-anexos/a14-retrato.txt`). O "landscape é o layout primário" não muda; o retrato deixou de ser a composição de C espremida.
...
| A4 | Abertura online: exatamente **1 + max(1, ⌈N/100⌉)** requests a `/api/*` (**2** com ambas as contas hoje) e lista visível do cache em < 1 s — *errata W1 (2026-09-14), medida no V1: com `N = 0` a fórmula antiga dá 1 e o app faz 2, e o app está certo, porque **descobrir que `N = 0` custa a request que devolve `N`**. `V1-ENCERRAMENTO.md` §5.1* | T1-R13 |
...
| | *Errata N2-PR1 (#309, 2026-09-16; N2-D8, div. 157) — o que o A7 passou a carregar e nunca mediu: o N1 pendurou o T1-R10 nele (`N1-PRECHECK.md:193`, "(A7)") e o V1 o dispensou pelo G1 lendo `cache write kind=setlists n=2 invalidated=0` no device (`V1-ENCERRAMENTO.md:122`) — um `0` **literal**, que nenhum sync podia mudar. O A7 passa a incluir o T1-R10 por escrito, medindo o **contador**: dois syncs sem mudança → `invalidated=0` nos dois conjuntos e o mesmo conjunto devolvido; `updated_at` diferente para um item → `invalidated=1` e o índice de busca sem a versão velha (texto do aceite do T1-R10, errata N2-PR1; CN em `apps/native/test/sync-t1r10.test.ts`, que reprovou 5 de 6 contra o código de antes — `N2-PR1-anexos/CN-t1r10.txt`).* | T1-R10 |
...
| A15 | Auto-scroll < 100 ms em texto, inerte em PDF **com o motivo revelado AO TOQUE**; zoom sem re-quebra; dark sheet em 1 tap | T1-R30, R31, R32 |
| | *Errata W1 (2026-09-14), do que o V1 mediu e do que ele supôs: a forma permanente do inerte é a do **ícone** (tinta `lineInfo`, desenho amputado), e o motivo aparece na linha acima da barra por `MOTIVO_MS = 2500` ao toque; o `accessibilityLabel` o carrega o tempo todo, então o leitor de tela nunca depende do toque (`V1-ENCERRAMENTO.md` §5.2). **E a errata E3 do `DESIGN-V1` está errada na outra metade**: ela afirma que o inativo é "tinta e, na árvore, `enabled=false`" — verdade para o botão `entrar` do S0, **falso para os três controles do palco**, que estão `enabled=true clickable=true` nos dumps do próprio V1 e no código (`StageScreen.tsx:807-827` não põe `disabled` nem `accessibilityState`). O conserto da árvore é da W2 — `W1-PRECHECK.md` div. 118.* | |
| | *Errata W2 (2026-09-15) — **a metade acima deixou de valer, e a data importa**: enquanto este arquivo dizia "`enabled=true`", isso era verdade; a partir da W2 é falso, e sem esta linha o PRD passaria a afirmar uma coisa falsa no dia do merge. O `Controle` do palco recebeu `accessibilityState={{ disabled: inativo === true }}` — **não** `disabled`, que impediria o `onPress` e quebraria a metade deste mesmo A15 que a errata W1 acabou de fixar. Medido no S3d do AVD: `auto-scroll`, `zoom-menos` e `zoom-mais` saem `enabled=false`, os quatro restantes `true`, e **`clickable=true` nos sete** — o motivo continua aparecendo ao toque (`linha-motivo` = "auto-scroll só em texto" e "pinça para zoom", capturado dentro dos `MOTIVO_MS`). Com isso a **errata E3 do `DESIGN-V1` deixa de ser falsa** e passa a descrever o app. `W2-anexos/W2-C-barra-e-arvore.txt`.* | |
