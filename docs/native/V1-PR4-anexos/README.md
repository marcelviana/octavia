...
> **DEPOIS** = `v1/pr4-s1` em `5f7d5c4` (commit 2), Metro do mesmo worktree.
...
`dumps-antes/` (9 estados) e `dumps-depois/` (10) — um `.xml` (`uiautomator
...
página 2, A21). `SHA256SUMS.txt` em cada diretório.
...
| # | Origem | O que é |
|---|---|---|
| **58** | **T** | O `node_modules` do checkout principal é de 10/09, anterior à #296: o Metro de lá **não resolve `react-native-svg`** ("Unable to resolve … from Icone.tsx", bundling failed, dev client em erro). Nada foi instalado no checkout principal (não é o desta sessão); o ANTES rodou do worktree em `82bfc3e`, cujo `apps/native/src`, `App.tsx`, `index.ts` e `app.json` têm diff **vazio** contra `origin/main` |
| **59** | **D** | As seis molduras do S1 desenham **só** o indicador `garantida` nos três cartões (a lista `setlists` do `telas.html` é a mesma nos três frames): `parcial`, `nunca sincronizada` e `baixando` (cartão e botão) **não têm moldura**. Implementados pela §6.4 (nome, tamanho, cores) e §6.1 (âmbar = não está pronta) — anexo D3 |
| **60** | **D** | A §6.1 diz "garantida em **tinta neutra**"; a moldura S1b pinta o ícone e o rótulo em `#777CE8` e a legenda diz "*garantida* de 28 em accentInk — mesma cor de antes". Seguida a **moldura** (a fonte desta PR); o corpo do README fica para errata |
...
| `V1-PR4-C-aparato-e-prod.txt` | 125 | `8b054bcd922caa739fc85c38ce080760560103c76debedf7449e3f27c8896aa8` |
...
| `dumps-depois/SHA256SUMS.txt` | 20 | `2e412c1426f0e9d40271bce48d8e9286c3e39e929333c571a295027742e88c24` |
