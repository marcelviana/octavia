...
describe('retryAfterFrom (T1-R4 / A3)', () => {
  it.fails('header Retry-After manda (é o autoritativo)', () => {
    expect(retryAfterFrom({ 'Retry-After': '30' }, null)).toBe(30)
    expect(retryAfterFrom({ 'retry-after': '30' }, ENVELOPE_429)).toBe(30)
  })

  it.fails('sem header, lê retryAfter do envelope RATE_LIMITED', () => {
    expect(retryAfterFrom({}, ENVELOPE_429)).toBe(30)
  })

  it.fails('sem header e sem envelope de 429 → null', () => {
  ...
  it.fails('header não-inteiro (data HTTP) e sem corpo → null', () => {
...
describe('rateLimitGate (T1-R4 / A3 — sem timer, relógio de quem chama)', () => {
  it.fails('bloqueia a família até untilMs', () => {
  ...
  it.fails('libera em untilMs e nextAllowedAt devolve o instante', () => {
    const gate = rateLimitGate()
  ...
  it.fails('famílias são independentes', () => {
    const gate = rateLimitGate()
    gate.block('content-read', 31_000)
