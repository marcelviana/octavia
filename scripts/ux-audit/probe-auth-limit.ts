...
import { resolveFaseDDir } from './fase-d-dirs'

config({ path: '.env.uxaudit', quiet: true })
config({ path: '.env.local', quiet: true })

const BASE_URL = process.env.UX_AUDIT_BASE_URL || 'https://octavia.rocks'
// Guard do registro histórico unificado (fase-d-dirs.ts): pós-fila A,
// QUALQUER rodada escreve em diretório efêmero, independente do alvo
// (antes o guard local só disparava com UX_AUDIT_BASE_URL setado).
const DATA_DIR = resolveFaseDDir('data')
const ATTEMPTS = 8
